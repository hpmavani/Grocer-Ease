...
image = Image.open("/Users/rithikapathuri/Downloads/Recipe-Finder/recipe-finder-backend/groceryReceiptPhoto.jpg")
...
geminiInput = "(" + current_ingredients+")"
...
#with open("prompt_response.json", "w") as jf: 
   #json.dump(response.to_dict(), jf)
   
   
cred = credentials.Certificate("recipe-finder-backend/recipe-finder-446fc-firebase-adminsdk-fbsvc-761ec286a4.json")
initialize_app(cred, {"databaseURL": "https://recipe-finder-446fc-default-rtdb.firebaseio.com/"})
ref = db.reference("recipes/list")
try:
    ref.set(response.to_dict())
